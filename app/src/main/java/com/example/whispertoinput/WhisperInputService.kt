...
class WhisperInputService : InputMethodService() {
    ...
    override fun onCreateInputView(): View {
        ...
        // Read alt theme preference (one-time sync read during keyboard creation)
        val isAltTheme = runBlocking {
            dataStore.data.map { preferences: Preferences ->
                preferences[ALT_THEME] ?: false
            }.first()
        }

        // Returns the keyboard after setting it up and inflating its layout
        val keyboardView = whisperKeyboard.setup(layoutInflater,
        ...
        // Add bottom padding for navigation bar so keyboard doesn't overlap it
        ViewCompat.setOnApplyWindowInsetsListener(keyboardView) { view, insets ->
            val navBarHeight = insets.getInsets(WindowInsetsCompat.Type.navigationBars()).bottom
            val basePadding = (8 * resources.displayMetrics.density).toInt() // 8dp base
            view.setPadding(
                view.paddingLeft,
                view.paddingTop,
                view.paddingRight,
                navBarHeight + basePadding
            )
            insets
        }
    ...
    override fun onWindowShown() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            // Check if alt theme was changed in settings since last shown
            val currentAltTheme = dataStore.data.map { preferences: Preferences ->
                preferences[ALT_THEME] ?: false
            }.first()
            whisperKeyboard.updateAltTheme(currentAltTheme)
